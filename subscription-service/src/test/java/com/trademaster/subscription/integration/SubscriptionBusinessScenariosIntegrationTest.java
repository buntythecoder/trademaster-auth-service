...
class SubscriptionBusinessScenariosIntegrationTest {
    ...
    void scenario1_completeTrialWorkflow_shouldSucceed() throws ExecutionException, InterruptedException {
        ...
        Result<Subscription, String> createResult = lifecycleService.createSubscription(
        ...
        createResult.match(
            subscription -> {
                ...
                // Activate trial after payment method added
                try {
                    UUID paymentTransactionId = UUID.randomUUID();
                    Result<Subscription, String> activateResult = lifecycleService.activateSubscription(
                        subscription.getId(), paymentTransactionId
                    ).get();

                    // Then - Subscription should be activated
                    assertThat(activateResult.isSuccess()).isTrue();
                    activateResult.onSuccess(activatedSub -> {
            ...
            error -> {
                throw new AssertionError("Trial subscription creation failed: " + error);
            }
        );
    }
    ...
    void scenario2_upgradeFromFreeToPremium_shouldSucceed() throws ExecutionException, InterruptedException {
        ...
        UUID subscriptionId = createResult.match(
            subscription -> subscription.getId(),
            error -> {
                throw new AssertionError("Free subscription creation failed: " + error);
            }
        );
        assertThat(subscriptionId).isNotNull();

        // When - User upgrades to AI Premium
        Result<Subscription, String> upgradeResult = upgradeService.upgradeSubscription(
        ...
        upgradeResult.onSuccess(upgradedSub -> {
    ...
    void scenario3_subscriptionCancellationWithGracePeriod_shouldSucceed() throws ExecutionException, InterruptedException {
        ...
        UUID subscriptionId = createResult.match(
            subscription -> subscription.getId(),
            error -> {
                throw new AssertionError("PRO subscription creation failed: " + error);
            }
        );
        assertThat(subscriptionId).isNotNull();

        // When - User requests cancellation
        Result<Subscription, String> cancelResult = cancellationService.cancelSubscription(
            subscriptionId, "User requested cancellation"
        ).get();

        // Then - Subscription should be cancelled
        assertThat(cancelResult.isSuccess()).isTrue();

        cancelResult.onSuccess(cancelledSub -> {
            ...
            assertThat(cancelledSub.getCancelledAt()).isNotNull();

            log.info("Subscription cancelled: {}", cancelledSub.getId());
        });
    ...
    void scenario4_tierDowngradeFromPremiumToPro_shouldSucceed() throws ExecutionException, InterruptedException {
        ...
        UUID subscriptionId = createResult.match(
            subscription -> subscription.getId(),
            error -> {
                throw new AssertionError("AI_PREMIUM subscription creation failed: " + error);
            }
        );
        assertThat(subscriptionId).isNotNull();

        // When - User downgrades to PRO tier
        Result<Subscription, String> downgradeResult = upgradeService.upgradeSubscription(
        ...
        downgradeResult.onSuccess(downgradedSub -> {
     ...
     *
     * NOTE: changeBillingCycle method not yet implemented in SubscriptionUpgradeService
     */
    @Test
    @org.junit.jupiter.api.Disabled("changeBillingCycle method not yet implemented")
    void scenario5_changeBillingCycleMonthlyToAnnual_shouldSucceed() throws ExecutionException, InterruptedException {
        ...
        // TODO: Implement changeBillingCycle in SubscriptionUpgradeService
        // When - User changes to annual billing
        // Result<Subscription, String> changeCycleResult = upgradeService.changeBillingCycle(
    ...
    void scenario6_multiUserConcurrentManagement_shouldSucceed() throws Exception {
        ...
        // Verify each subscription using findActiveByUserId
        for (UUID userId : userIds) {
            Optional<Subscription> subscription = subscriptionRepository.findActiveByUserId(
                userId, List.of(SubscriptionStatus.ACTIVE, SubscriptionStatus.PENDING)
            );
    ...
    void scenario7_reactivateAfterCancellation_shouldSucceed() throws ExecutionException, InterruptedException {
        ...
        Result<Subscription, String> cancelResult = cancellationService.cancelSubscription(
            subscriptionId, "Testing reactivation workflow"
        ).get();

        assertThat(cancelResult.isSuccess()).isTrue();
        SubscriptionStatus cancelledStatus = cancelResult.match(
            subscription -> subscription.getStatus(),
            error -> {
                throw new AssertionError("Cancellation failed: " + error);
            }
        );
        assertThat(cancelledStatus).isEqualTo(SubscriptionStatus.CANCELLED);

        // When - User reactivates the subscription
        UUID paymentTransactionId = UUID.randomUUID();
        Result<Subscription, String> reactivateResult = lifecycleService.activateSubscription(
        ...
        reactivateResult.onSuccess(reactivatedSub -> {
