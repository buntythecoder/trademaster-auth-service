...
public class PortfolioLogger {
    ...
    public void logPerformanceMetrics(String operation, long durationMs, boolean success,
        ...
        // Functional approach: fold additionalMetrics into logBuilder
        var finalLogBuilder = Optional.ofNullable(additionalMetrics)
            .map(Map::entrySet)
            .orElse(Set.of())
            .stream()
            .reduce(logBuilder, 
                (builder, entry) -> builder.addKeyValue(entry.getKey(), entry.getValue()),
                (b1, b2) -> b1); // Identity combiner since we're not using parallel
        
        finalLogBuilder.log("Operation performance metrics recorded");
    }
    ...
    public void logAuditEvent(String eventType, Long userId, Long portfolioId,
        ...
        // Functional approach: fold auditData into logBuilder
    ...
    public void logBusinessMetrics(String metricType, Map<String, Number> metrics) {
        ...
        // Functional approach: fold metrics into logBuilder
        var finalMetricsLogBuilder = metrics.entrySet().stream()
            .reduce(logBuilder,
                (builder, entry) -> builder.addKeyValue("metric_" + entry.getKey(), entry.getValue()),
                (b1, b2) -> b1);
        
        finalMetricsLogBuilder.log("Business metrics recorded for analytics");
    }
    ...
    public void logWithStructuredData(String message, String logLevel, 
        ...
        // Functional approach: fold structured data into logBuilder
        var finalStructuredLogBuilder = structuredData.entrySet().stream()
            .reduce(logBuilder,
                (builder, entry) -> builder.addKeyValue(entry.getKey(), entry.getValue()),
                (b1, b2) -> b1);
